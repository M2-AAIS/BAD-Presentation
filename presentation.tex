...
Combinaison des méthodes de la dichotomie, de la sécante et l'interpolation quadratique inverse pour en utiliser tous leurs avantages.
  ...
  &+ \frac{\textsl{•}bf(a)f(c)}{(f(b)-f(a))(f(b)-f(c))} + \frac{cf(a)f(b)}{(f(c)-f(a))(f(c)-f(b))} \\
         \end{eqnarray*}
...
\begin{frame}
  \frametitle{Intégration hors du domaine stable -- schéma explicite}
  \uncover<1->{
    \begin{equation}\begin{aligned}
...
\begin{frame}
  ...
  \only<2>{
    \begin{figure}
      \includegraphics{figures/Ltot_fonction_t_zoom.pdf}
      \caption{Évolution température en fonction du temps, $\dot{M}_{t=0}$ variable.}
    \end{figure}  
  }
\end{frame}
